// Prayer time calculation using PrayTime.js (MIT License)
// Source: https://praytimes.org/code/praytime/src/praytime.js
// Lightly adapted into a module for this app.
...
class PrayTime {
  ...
  utcOffset(utcOffset = 'auto') {
    if (typeof utcOffset === 'number' && Math.abs(utcOffset) < 16) utcOffset *= 60;
  ...
  angleTime(angle, time, direction = 1) {
    ...
    const diff =
      this.arccos(numerator / (this.cos(lat) * this.cos(decl))) / 15;
...
// Helper: compute today's prayer times for a given location and timezone
// Returns an object: { schedule: [{ name, time, dateTime }...], raw: PrayTimeResult }
...
export function getTodayPrayerTimes({
  ...
  const prayTime = new PrayTime(method);
  const today = new Date();

  // Get times as timestamps (format 'x' returns epoch milliseconds)
  ...
  // Internal keys in PrayTime are lowercase
  const keyOrder = ['fajr', 'dhuhr', 'asr', 'maghrib', 'isha'];
  const displayNames = {
    fajr: 'Fajr',
    dhuhr: 'Dhuhr',
    asr: 'Asr',
  ...
  const schedule = keyOrder
    .map((key) => {
      const value = times[key];
      // Skip invalid or missing times
      if (typeof value !== 'number' || !Number.isFinite(value)) {
        return null;
      }

      // value is UTC timestamp in milliseconds
      const dt = DateTime.fromMillis(value, { zone: 'utc' }).setZone(timeZoneId);
